...
def get_drive_hardware_info() -> Dict[str, Dict]:
    ...
    disks = _query_wmi("cimv2", "SELECT DeviceID, Model, MediaType, InterfaceType, PNPDeviceID, Status FROM Win32_DiskDrive")
    failure_rows = _query_wmi("wmi", "SELECT InstanceName, PredictFailure FROM MSStorageDriver_FailurePredictStatus")
    predict_failure = any(bool(row.get("PredictFailure")) for row in failure_rows)

    for disk in disks:
        model = str(disk.get("Model") or "Disk Drive").strip()
        media_type = str(disk.get("MediaType") or "")
        interface_type = str(disk.get("InterfaceType") or "")
        pnp_id = str(disk.get("PNPDeviceID") or "")
        smart_status = "Warning" if predict_failure else str(disk.get("Status") or "OK")
        info = {
            "model": model,
            "bus_type": interface_type or "Unknown",
            "media_type": _infer_media_type(model, media_type, interface_type, pnp_id),
        ...
        for logical in logicals:
            ...
            if letter:
                hardware[f"{letter}\\"] = info

    if not hardware and disks:
        first = disks[0]
        model = str(first.get("Model") or "Disk Drive").strip()
        media_type = str(first.get("MediaType") or "")
        interface_type = str(first.get("InterfaceType") or "")
        pnp_id = str(first.get("PNPDeviceID") or "")
        info = {
            "model": model,
            "bus_type": interface_type or "Unknown",
            "media_type": _infer_media_type(model, media_type, interface_type, pnp_id),
            "smart_status": "Warning" if predict_failure else str(first.get("Status") or "OK"),
            "temperature": None,
        }
        for partition in get_drive_partitions_info():
            hardware[partition["drive"]] = info
