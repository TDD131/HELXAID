---
description: Build Game Launcher Project (--onefile and --onedir mode)
---
...
> **IMPORTANT**: Close HELXAID completely before building.  
> Running HELXAID during a build causes two problems:
> 1. **File lock conflict**: PyInstaller tries to read `launcher.py` and its imports but the running process may hold a lock on related AppData files.
> 2. **Data race**: If HELXAID saves `game_library.json` while PyInstaller is bundling the AppData directory, the build result may include a stale or mid-write copy of the data.
>
> Close HELXAID via the system tray icon → "Exit" before proceeding.
